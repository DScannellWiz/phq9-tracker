...
Later on August 31, Daniel transferred the exact candidate ZIP to a volunteer's separate Windows computer. The volunteer received and extracted the ZIP on that machine, launched and used the packaged application, and played around in it; Daniel reported that all exercised behavior worked. No Python, Tkinter, or other development runtime preparation was performed. This closes the identified separate-Windows extraction-and-launch gate to the extent observed. It does not claim a pristine VM or freshly installed Windows image, specific SmartScreen or other security-prompt behavior, or exact persistence/output subtests. Work's installed-mode desktop-control pass, Daniel's development-workstation EXE/batch test, and a volunteer's separate-machine test remain three distinct bodies of evidence.
